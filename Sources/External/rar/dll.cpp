...
int PASCAL RARReadHeaderEx(HANDLE hArcData,struct RARHeaderDataEx *D)
{
  ...
  try
  {
    ...
    {
      int Code=RARProcessFile(hArcData,RAR_SKIP,NULL,NULL,0);
      if (Code==0)
        return RARReadHeaderEx(hArcData,D);
      else
        return Code;
...
int PASCAL ProcessFile(HANDLE hArcData,int Operation,char *DestPath,char *DestName,wchar *DestPathW,wchar *DestNameW,uint ReadLength)
{
  DataSet *Data=(DataSet *)hArcData;
  try
  {
    ...
    else
    {
      ...
      *Data->Cmd.DllDestName=0;
      ...
      bool Repeat=false;
      Data->Extract.ExtractCurrentFile(Data->Arc,Data->HeaderSize,Repeat,ReadLength);
...
int PASCAL RARProcessFile(HANDLE hArcData,int Operation,char *DestPath,char *DestName, uint ReadLength)
{
  return(ProcessFile(hArcData,Operation,DestPath,DestName,NULL,NULL,ReadLength));
}


int PASCAL RARProcessFileW(HANDLE hArcData,int Operation,wchar *DestPath,wchar *DestName, uint ReadLength)
{
  return(ProcessFile(hArcData,Operation,NULL,NULL,DestPath,DestName,ReadLength));
}
