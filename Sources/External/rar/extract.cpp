...
EXTRACT_ARC_CODE CmdExtract::ExtractArchive()
{
  ...
  while (1)
  {
    ...
    bool Repeat=false;
    if (!ExtractCurrentFile(Arc,Size,Repeat,0))
      if (Repeat)
      {
...
bool CmdExtract::ExtractCurrentFile(Archive &Arc,size_t HeaderSize,bool &Repeat,uint ReadLength)
{
      ...
      if (LinkEntry)
      {
        ...
        if (!Arc.FileHead.SplitBefore)
          if (Arc.FileHead.Method==0)
            UnstoreFile(DataIO,Arc.FileHead.UnpSize,ReadLength);
          else
          {
...
void CmdExtract::UnstoreFile(ComprDataIO &DataIO,int64 DestUnpSize,uint ReadLength)
{
  ...
  uint BufferSize = 0x40000;
  if (ReadLength > 0) {
    BufferSize = Min(ReadLength, BufferSize);
  }
  ...
  while (1)
  {
    ...
    UnpackedSize += Code;
    if (ReadLength != 0 && UnpackedSize >= ReadLength) {
      break;
    }
    if (DestUnpSize>=0)
      DestUnpSize-=Code;
